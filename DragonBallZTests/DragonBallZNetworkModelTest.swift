...
final class DragonBallZNetworkModelTest: XCTestCase{
    ...
    override func setUp() {
        ...
        DragonBallZNetworkModel.session = session
    }
    ...
    func testGetHeroesList(){
        DragonBallZNetworkModel.token = "abcdefg1234567"
        let heroeArray:[Hero] = [
        ...
        DragonBallZNetworkModelTestURLProtocol.requestHandler = { request in
            XCTAssertEqual(request.httpMethod, "POST")
            XCTAssertEqual(request.url!.scheme, "https")
            XCTAssertEqual(request.value(forHTTPHeaderField: "Authorization"), "Bearer \(DragonBallZNetworkModel.token)")
    ...
    func testGetHeroeTransformations(){
        ...
        DragonBallZNetworkModel.getTransformationsList(heroId: heroID) { data in
            XCTAssertNotNil(data)
            expectation.fulfill()
        }
        wait(for: [expectation], timeout: 10)
